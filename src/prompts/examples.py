...
def get_few_shots(file_path: str) -> FewShotPromptTemplate:
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if line:  # Skip empty lines
                    ...
                    examples.append(
                        {"input": example["user"], "output": example["assistant"]}
                    )
    except FileNotFoundError:
        raise FewShotExamplesException(
            f"Предупреждение: Файл с примерами не найден по пути {file_path}"
        )
    except json.JSONDecodeError:
        raise FewShotExamplesException(
            f"Предупреждение: Неверный формат JSON в {file_path}"
        )
    except KeyError:
        raise FewShotExamplesException(
            f"Предупреждение: Отсутствуют обязательные поля 'user' или 'assistant' в {file_path}"
        )
    ...
    example_prompt = PromptTemplate.from_template("user: {input}\nassistant: {output}")

    example_selector = SemanticSimilarityExampleSelector.from_examples(
        examples=examples, embeddings=embeddings, vectorstore_cls=Chroma, k=1
    )
